...
如果你下载的是 GitHub Release 里的 `datamind-context-plugin-cursor-1.0.0.zip`，发布包已经内置干净的 DataMind runtime 源码。普通用户不需要先单独 clone DataMind 主仓库，直接运行：
...
```text
Settings -> Plugins -> Local Plugins
```

里启用 `datamind-context`。
...
当你说出稳定偏好、工作习惯、资料处理规则时，Cursor 会自动记到全局 session：
