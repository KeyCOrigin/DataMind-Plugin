...
DataMind Context 是 DataMind 的 Claude Code 插件。它让 Claude Code 可以使用你本地的资料文件夹，自动建立 RAG / GraphRAG 知识库，生成轻量 Markdown Wiki，并记住长期有价值的偏好、决策和工作流程。
...
如果插件位于 DataMind 仓库内的 `plugins/datamind-context/`，进入这个目录运行：
...
`install.sh` 会自动调用 `claude plugin marketplace add` 和 `claude plugin install`。

然后重启 Claude Code（已开启的会话可运行 `/reload-plugins`）。验证：

```bash
claude mcp list  # 应显示 plugin:datamind-context:datamind-context ✓ Connected
```
...
安装脚本会在 `vendor/datamind/.venv` 中建 venv 并装依赖，然后调用 Claude Code 的 plugin marketplace 完成注册。Claude Code 会把插件复制到自己的 plugin cache（`~/.claude-internal/plugins/cache/datamind/...`），更新插件时会自动管理新版本。
