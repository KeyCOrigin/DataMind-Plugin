# DataMind Context 超简单使用教程

这版默认会自动处理 session 和 profile。你平时不用写 session。

同时会为每个资料库自动生成一个轻量 Markdown Wiki，用来沉淀索引、来源页、偏好和更新日志。

新版还会额外维护一个 MemOS-inspired 结构化记忆索引，用来更精准地召回偏好、决策、工作流总结和生成的 skill。它不替代原有 Markdown 记忆；旧的 `events.jsonl`、`memory.md`、`skill.md` 和 Wiki 都会继续保留。

## 安装成 Claude Code 插件

如果你下载的是 GitHub Release 里的 `datamind-context-plugin-claude-code-1.0.0.zip`，发布包已经内置干净的 DataMind runtime 源码。普通用户不需要先单独 clone DataMind 主仓库，直接运行：

```bash
unzip datamind-context-plugin-claude-code-1.0.0.zip
cd datamind-context
./install.sh
```
...
安装后编辑安装脚本提示的 `.env` 文件，填入 LLM 和 embedding 配置。

如果你是从 DataMind 主仓库源码安装，进入插件目录运行：

```bash
cd /path/to/datamind-context
./install.sh
```

然后重启 Claude Code（或在已开会话里 `/reload-plugins`）。
...
如果你只是日常使用，可以先跳过这一节。发布给团队或其他用户时，建议保留这一节。
...
它记录你和 Claude Code 互动中有长期价值的内容，比如：
...
当你说出稳定偏好、工作习惯、资料处理规则时，Claude Code 会自动记到全局 session：
...
查询时，DataMind 会先带上 Wiki、相关结构化记忆和全局 Markdown 记忆，再进入 RAG 或 GraphRAG。如果结构化数据库不可用，会自动退回旧的 Wiki + Markdown 记忆路径。
