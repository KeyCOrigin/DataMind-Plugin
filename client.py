...
class McpClient:
    ...
    def __init__(self, command: str, args: Sequence[str] = (), *,
        ...
        # Keep internal calls below the host MCP timeout so a failed DataPlane
        # request becomes a structured tool error instead of an orphaned write.
        self._timeout = max(1.0, float(os.environ.get("DATAMIND_INTERNAL_TOOL_TIMEOUT_SEC", "90")))
    ...
    async def _read_stderr(self) -> None:
        ...
        while True:
            ...
            if not line:
                return
            # DataPlane logs must never contain tokens or document bodies.
            # Keep the stream drained so a noisy child cannot deadlock.
            _log.info("dataplane: %s", line.decode(errors="replace").rstrip())
    ...
    async def _rpc(self, method: str, params: dict[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            raise TimeoutError(f"DataPlane tool call timed out after {self._timeout:.0f}s") from None
