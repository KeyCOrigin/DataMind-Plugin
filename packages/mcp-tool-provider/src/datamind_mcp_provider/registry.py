...
_EXTERNAL_WRITE = {"kb_add_text", "kb_add_file", "db_import_records", "db_import_csv", "graph_upsert_triples"}


class McpToolProvider:
    def __init__(self, client: McpClient, *, context: dict[str, Any], store_client: McpClient | None = None,
                 store_allowed: set[str] | None = None) -> None:
        self.client = client
        self.store_client = store_client or client
        self.store_allowed = store_allowed or _WRITE
    ...
    async def store_tools(self, context: Any) -> ToolRegistry:
        registry = registry_from_tools(await self._catalog(self.store_client), self.store_client, context=self.context, allowed=self.store_allowed)
