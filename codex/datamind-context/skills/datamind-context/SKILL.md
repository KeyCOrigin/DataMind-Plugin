...
description: Use DataMind from Codex to refresh a local folder into a RAG/GraphRAG knowledge base, then query document knowledge or graph relationships.
---

# DataMind Context

Use this skill when the user wants Codex to use DataMind, query a registered local folder, refresh a knowledge base, or ask about knowledge graph relationships in local documents.
...
- `questions/` contains high-value questions captured from Codex conversations.
...
3. Use `datamind_remember` when the user wants Codex to remember a preference, decision, repeated workflow, or important interaction, and also for high-value habits detected automatically. This also writes Wiki preferences.
...
- `profile` is the stable handle Codex can reuse after a folder is refreshed.
